...
const constructGameElement = (game) => {
  ...
  if (indexOfUser !== -1) {
    ...
    if (user.status === "ready") {
      const span_rejoin = document.createElement("span");
      span_rejoin.className = "btn badge bg-danger rounded-spill mx-1";
      span_rejoin.id = "game-" + game.game_id + "-rejoin";
      span_rejoin.onclick = reJoin;
      span_rejoin.innerHTML = "rejoin";
      buttons_div.appendChild(span_rejoin);
    } else {
      // The game is playing
