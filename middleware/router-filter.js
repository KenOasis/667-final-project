...
const lobbyRoutesPaths = lobbyRoutes.routes.stack.map(
  (layer) => lobbyRootPath + layer.route.path
);
const gameRootPath = "/game";
const gameRoutesPaths = gameRoutes.routes.stack.map(
  (layer) => gameRootPath + layer.route.path
);

const loggedInFilter = [].concat(manuLoggedIn);
const loggedOutFilter = manuLoggedOut
  .concat(lobbyRoutesPaths)
  .concat(gameRoutesPaths);
const routerFilter = (req, res, next) => {
  const path = req.url;
  const isLoggedIn = req.session.isLoggedIn === true ? true : false;

  const isForbidden =
    (loggedInFilter.includes(path) && isLoggedIn) ||
    (loggedOutFilter.includes(path) && !isLoggedIn);
  if (isForbidden) {
